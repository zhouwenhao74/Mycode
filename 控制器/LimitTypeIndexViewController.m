//
//  LimitTypeIndexViewController.m
//  爱限免V5
//
...
@interface LimitTypeIndexViewController ()<ZTableViewDataSource>
{
    //存放显示数据
...
@implementation LimitTypeIndexViewController
- (void)dealloc
{
    [self ai_ViewUnload];
...
#pragma -mark TableViewDataSource 方法
...
-(void)getDataFromNetWithArray:(NSMutableArray *)dateArray withType:(LoadDataType)loadData withFinishCompletion:(void (^)(BOOL finished)) completion
{
    if (loadData == ReloadDataType) {
        Ex_Page = 1;
        [QFHttpRequest requestWithUrlString:LIMIT_URL(Ex_Page, Ex_Number) finish:^(NSData *data)
         {
