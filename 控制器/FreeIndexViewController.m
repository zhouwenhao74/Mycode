...
@interface FreeIndexViewController ()<ZTableViewDataSource>
{
    //存放显示数据
...
@implementation FreeIndexViewController
- (void)dealloc
{
    [self ai_ViewUnload];
...
{
    LimitTypeIndexIAppInformationModel *iAppInformationModel = [[LimitTypeIndexIAppInformationModel alloc] initWithDictionary:_dataSourceArray[indexPath.row]];
    AppInformationViewController *appInformationViewController = [[AppInformationViewController alloc] initWithAppId:iAppInformationModel.appIDStr iconUrlString:iAppInformationModel.leftOfImageViewImageUrl appNameStr:iAppInformationModel.appName];
...
-(void)getDataFromNetWithArray:(NSMutableArray *)dateArray withType:(LoadDataType)loadData withFinishCompletion:(void (^)(BOOL finished)) completion
{
    if (loadData == ReloadDataType) {
        Ex_Page = 1;
        [QFHttpRequest requestWithUrlString:FREE_URL(Ex_Page, Ex_Number) finish:^(NSData *data)
         {
