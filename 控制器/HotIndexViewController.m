...
@interface HotIndexViewController ()<ZTableViewDataSource>
{
    //存放显示数据
...
@implementation HotIndexViewController
- (void)dealloc
{
    [self ai_ViewUnload];
...
-(void)getDataFromNetWithArray:(NSMutableArray *)dateArray withType:(LoadDataType)loadData withFinishCompletion:(void (^)(BOOL finished)) completion
{
    if (loadData == ReloadDataType) {
        Ex_Page = 1;
        [QFHttpRequest requestWithUrlString:HOT_URL(Ex_Page, Ex_Number) finish:^(NSData *data)
         {
